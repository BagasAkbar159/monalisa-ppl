        ...
        <form action="{{ route('admin.drivers.update', $driver->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input
                    ...
                    value="{{ old('license_expiry_date', optional($driver->license_expiry_date)->format('Y-m-d')) }}"
            ...
            <div>
                ...
                >
                    <option value="1" {{ old('is_active', $driver->is_active) == 1 ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('is_active', $driver->is_active) == 0 ? 'selected' : '' }}>Nonaktif</option>
                </select>
                @error('is_active')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
