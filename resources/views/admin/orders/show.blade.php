...
<div class="mx-auto max-w-4xl space-y-6">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            ...
            <p class="app-page-subtitle">Informasi lengkap pesanan {{ $order->order_code }}.</p>
        </div>

        <div class="flex flex-wrap gap-2">
            @if ($order->status !== 'dibatalkan')
                <a href="{{ route('admin.orders.edit', $order) }}" class="app-btn-accent">Edit</a>
            @endif
            <a href="{{ route('admin.orders.index') }}" class="app-btn-secondary">Kembali</a>
        </div>
    </div>
        ...
        <div class="app-card-body">
            ...
            <div class="mt-6 grid gap-5 md:grid-cols-2">
                ...
                <div class="rounded-2xl bg-slate-50 p-4">
                    ...
                    <p class="mt-1 font-bold text-slate-900">{{ $order->customer->company_name ?? '-' }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ $order->customer->user->name ?? '-' }}</p>
                </div>
                ...
                <div class="rounded-2xl bg-[#102C4D] p-4 text-white">
                ...
                <div class="rounded-2xl bg-slate-50 p-4 md:col-span-2">
                    ...
                    <p class="mt-1 font-bold text-slate-900">{{ $order->creator->name ?? '-' }}</p>
                </div>
            </div>

            @if (! in_array($order->status, ['selesai', 'dibatalkan'], true))
                <div class="mt-6 border-t border-slate-200 pt-6">
                    <form action="{{ route('admin.orders.destroy', $order) }}" method="POST" onsubmit="return confirm('Yakin ingin membatalkan pesanan ini?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="app-btn-danger">Batalkan Pesanan</button>
                    </form>
                </div>
            @endif
        </div>
